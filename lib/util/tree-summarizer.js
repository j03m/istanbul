...
TreeSummary.prototype = {
    ...
    convertToTree: function (summaryMap, arrayPrefix) {
        ...
        this.calculateMetrics(root, true);
        this.root = root;
        this.map = {};
    ...
    calculateMetrics: function (entry, percentOnly) {
        var that = this,
            fileChildren;
        ...
        var method;
        if (percentOnly){
            method = utils.mergeSummaryObjects;
        }else{
            method = utils.mergeSummaryObjectsPCT;
        }

        entry.metrics = method.apply(
            null,
        ...
        if (fileChildren.length > 0) {
            entry.packageMetrics = utils.mergeSummaryObjectsPCT.apply(
                null,
